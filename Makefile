...
run: build
	$(OPAM_EXEC) $(DUNE) exec -- scope $(files)

debug: build
	$(OPAM_EXEC) ocamldebug _build/default/scope/main.bc
...
<meta content="https://github.com/CharlesAverill/scope" property="og:url" />
endef
...
docs: cleandocs build
	...
	mv $(DOCS_PATH)scope/scope.html $(DOCS_PATH)index.html
	mv $(DOCS_PATH)scope $(DOCS_PATH)module
